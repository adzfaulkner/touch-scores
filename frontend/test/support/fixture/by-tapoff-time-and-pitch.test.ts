...
describe('Aggregates raw data', () => {
    it('Should aggregate the raw data as expected', (): void => {
        ...
        it(dates, times, pitches, fixtures, teams, stages, refs, dateFixtureCount)
        ...
        assert.lengthOf(fixtures, 1)
        assert.lengthOf(Array.from(Array.from(fixtures.values())[0].keys()), 14)
        assert.equal(Array.from(dateFixtureCount.values())[0], 72)
